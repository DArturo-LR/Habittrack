...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter = HabitAdapter(
            ...
            { habit ->
                ...
                if (habit.progress < habit.goal && habit.lastCompletedDate != today) {
                    ...
                    // Optimistic update
                    allHabits = allHabits.map { if (it.id == updatedHabit.id) updatedHabit else it }
    ...
    private fun setupFilters() {
        findViewById<Button>(R.id.btnTodos).setOnClickListener { adapter.updateData(allHabits) }
        findViewById<Button>(R.id.btnSalud).setOnClickListener { adapter.updateData(allHabits.filter { it.category == "Salud" }) }
        findViewById<Button>(R.id.btnEstudio).setOnClickListener { adapter.updateData(allHabits.filter { it.category == "Estudio" }) }
        findViewById<Button>(R.id.btnPasatiempo).setOnClickListener { adapter.updateData(allHabits.filter { it.category == "Pasatiempo" }) }
        findViewById<Button>(R.id.btnOtros).setOnClickListener { adapter.updateData(allHabits.filter { it.category == "Otros" }) }
    }
